...
URL_tlcm8081 ="http://localhost:8081" 
...
def vendainserir ():
    if request.method == 'POST':
        ...
        respostatll8081 = request.post(url='http://localhost:8081/verificar_prato'+ txt)
        txt = respostatll8081.content
        listatll8081 = json.loads(txt)
        print(listatll8081)

        txt = json.dums({"nome": nome, "cpf": cpf})
        respostatll8083 = request.post(url='http://localhost:8083/verificar_prato'+ txt)
        txt = respostatll8083.content
        listatll8083 = json.loads(txt)
        print(listatll8083)
...
app.run(port=8082, use_reloader=True)
