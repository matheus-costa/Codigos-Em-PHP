...
URL_tll5002 ="http://localhost:5002/verifica" 
URL_tlsp5003 ="http://localhost:5003/verifica"
...
#SQL QUE VERIFICA SE O CLIENTE DESTE RESTAURANTE COMPROU EM OUTRO RESTAURANTE
    sql = f"select v.id, p.nome pnome, c.nome cnome, p.valor pvalor, v.valor vvalor, v.data 
            ...
            where c.cpf ='{cpf}' and p.nome ='{nome}' ;
             "
    conexao.close()
    resultado = conexao.execute(sql).fetchall() 
    if len(resultado) > 0: #VERIFICO SE HOUVE RETORNO DA BUSCA POR ESSE CLIENTE    
        txt = json.dumps({"nome": nome, "cpf": cpf})
        urlTXt = URL_tll5002
        resp = requests.post(url=urlTXt, data=txt)
        txt = resp.content
        
        txt2 = json.dumps({"nome": nome, "cpf": cpf})
        urlTXt2 = URL_tlsp5003
        resp = requests.post(url=urlTXt2, data=txt2)
        txt2 = resp.content
...
def vendalistar ():#NA HORA DE LISTAR EU CHAMO O VERIFICAR, SE RETORNAR EU SOBRESCREVO O LISTAR
    conexao = sqlite3.connect('banco')
    ...
    obj = json.loads(txt)
    if(obj = true) #VERIFICANDO O QUE O OBJETO RETORNA  
